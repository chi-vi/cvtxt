...
class Cvtxt::Split
  ...
  def initialize(@encoding = "GBK", @split_mode = 1)
  end

  def set_encoding(encoding : String)
    case encoding.downcase.tr("-", "")
    when "utf8", "u8"               then self.encoding = "UTF-8"
    when "utf16", "u16"             then self.encoding = "UTF-16"
    when "gbk", "gb18030", "gb2312" then self.encoding = "GBK"
    else                                 raise "Chưa hỗ trợ mã hoá ký tự #{encoding}"
    end
  end
